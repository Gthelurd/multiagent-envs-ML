...
class PPOConfig:
    def __init__(self) -> None:
        ...
        self.batch_size  = 1024      # minibatch size ori:256
        self.gamma = 0.99      # discount factor ori:0.99
        self.lr = 1e-4          # learning rate ori:3e-4
        self.eps_clip = 0.2      # clip parameter for PPO(更新的幅度)
        self.K_epochs = 100        # number of epochs for PPO update
        self.update_every = 1   # Learn every UPDATE_EVERY time steps.
        self.train_eps = 10000     # total training time
        self.train_steps = 200     # time steps per training loop
        self.eval_eps = 1024         
        self.eval_steps = 200
        self.eps_start = 1.0 #探索率初始值
        self.eps_decay = 0.995 #探索率衰减
        self.eps_end = 1e-5 #探索率终止值（最小值）
        self.device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.frames = 16         # stack frames
...
def train(cfg:PPOConfig,env,agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while True:
            ...
            next_obs_n, reward_n, done_n, info_n = env.step(act.cpu().squeeze(0).numpy())
            ...
            print(f"Episode:{i_ep+1}/{cfg.train_eps}, step {i_step}, action: {action} Reward:{ep_reward:.3f}, Info:{info_n}", end="\r")
        ...
        if i_ep % 8 == 0:
            ...
            np.savetxt(cfg.model_path+'reward.txt',rewards)
            np.savetxt(cfg.model_path+'ma_reward.txt',ma_rewards)
...
if __name__ == "__main__":
    ...
    if cfg.train:
        ...
        if cfg.load_model:
            print(">>>>>>>>>>load model<<<<<<<<<<<<<<<")
            agent.load(path=cfg.model_path,i_ep=loadstep)
